# coding: utf-8

from __future__ import absolute_import
from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from swagger_server.models.base_model_ import Model
from swagger_server import util


class Datapoint(Model):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """

    def __init__(self, sentence_id: int=None, sentence: str=None, rule1: bool=None, rule2: bool=None, rule3: bool=None, rule4: bool=None, rule5: bool=None, date_added: str=None):  # noqa: E501
        """Datapoint - a model defined in Swagger

        :param sentence_id: The sentence_id of this Datapoint.  # noqa: E501
        :type sentence_id: int
        :param sentence: The sentence of this Datapoint.  # noqa: E501
        :type sentence: str
        :param rule1: The rule1 of this Datapoint.  # noqa: E501
        :type rule1: bool
        :param rule2: The rule2 of this Datapoint.  # noqa: E501
        :type rule2: bool
        :param rule3: The rule3 of this Datapoint.  # noqa: E501
        :type rule3: bool
        :param rule4: The rule4 of this Datapoint.  # noqa: E501
        :type rule4: bool
        :param rule5: The rule5 of this Datapoint.  # noqa: E501
        :type rule5: bool
        :param date_added: The date_added of this Datapoint.  # noqa: E501
        :type date_added: str
        """
        self.swagger_types = {
            'sentence_id': int,
            'sentence': str,
            'rule1': bool,
            'rule2': bool,
            'rule3': bool,
            'rule4': bool,
            'rule5': bool,
            'date_added': str
        }

        self.attribute_map = {
            'sentence_id': 'sentenceID',
            'sentence': 'sentence',
            'rule1': 'rule1',
            'rule2': 'rule2',
            'rule3': 'rule3',
            'rule4': 'rule4',
            'rule5': 'rule5',
            'date_added': 'dateAdded'
        }

        self._sentence_id = sentence_id
        self._sentence = sentence
        self._rule1 = rule1
        self._rule2 = rule2
        self._rule3 = rule3
        self._rule4 = rule4
        self._rule5 = rule5
        self._date_added = date_added

    @classmethod
    def from_dict(cls, dikt) -> 'Datapoint':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The datapoint of this Datapoint.  # noqa: E501
        :rtype: Datapoint
        """
        return util.deserialize_model(dikt, cls)

    @property
    def sentence_id(self) -> int:
        """Gets the sentence_id of this Datapoint.

        Unique ID of the sentence that was submitted for review  # noqa: E501

        :return: The sentence_id of this Datapoint.
        :rtype: int
        """
        return self._sentence_id

    @sentence_id.setter
    def sentence_id(self, sentence_id: int):
        """Sets the sentence_id of this Datapoint.

        Unique ID of the sentence that was submitted for review  # noqa: E501

        :param sentence_id: The sentence_id of this Datapoint.
        :type sentence_id: int
        """

        self._sentence_id = sentence_id

    @property
    def sentence(self) -> str:
        """Gets the sentence of this Datapoint.

        The user submitted sentence to be reviewed  # noqa: E501

        :return: The sentence of this Datapoint.
        :rtype: str
        """
        return self._sentence

    @sentence.setter
    def sentence(self, sentence: str):
        """Sets the sentence of this Datapoint.

        The user submitted sentence to be reviewed  # noqa: E501

        :param sentence: The sentence of this Datapoint.
        :type sentence: str
        """
        if sentence is None:
            raise ValueError("Invalid value for `sentence`, must not be `None`")  # noqa: E501

        self._sentence = sentence

    @property
    def rule1(self) -> bool:
        """Gets the rule1 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :return: The rule1 of this Datapoint.
        :rtype: bool
        """
        return self._rule1

    @rule1.setter
    def rule1(self, rule1: bool):
        """Sets the rule1 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :param rule1: The rule1 of this Datapoint.
        :type rule1: bool
        """
        if rule1 is None:
            raise ValueError("Invalid value for `rule1`, must not be `None`")  # noqa: E501

        self._rule1 = rule1

    @property
    def rule2(self) -> bool:
        """Gets the rule2 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :return: The rule2 of this Datapoint.
        :rtype: bool
        """
        return self._rule2

    @rule2.setter
    def rule2(self, rule2: bool):
        """Sets the rule2 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :param rule2: The rule2 of this Datapoint.
        :type rule2: bool
        """
        if rule2 is None:
            raise ValueError("Invalid value for `rule2`, must not be `None`")  # noqa: E501

        self._rule2 = rule2

    @property
    def rule3(self) -> bool:
        """Gets the rule3 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :return: The rule3 of this Datapoint.
        :rtype: bool
        """
        return self._rule3

    @rule3.setter
    def rule3(self, rule3: bool):
        """Sets the rule3 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :param rule3: The rule3 of this Datapoint.
        :type rule3: bool
        """
        if rule3 is None:
            raise ValueError("Invalid value for `rule3`, must not be `None`")  # noqa: E501

        self._rule3 = rule3

    @property
    def rule4(self) -> bool:
        """Gets the rule4 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :return: The rule4 of this Datapoint.
        :rtype: bool
        """
        return self._rule4

    @rule4.setter
    def rule4(self, rule4: bool):
        """Sets the rule4 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :param rule4: The rule4 of this Datapoint.
        :type rule4: bool
        """
        if rule4 is None:
            raise ValueError("Invalid value for `rule4`, must not be `None`")  # noqa: E501

        self._rule4 = rule4

    @property
    def rule5(self) -> bool:
        """Gets the rule5 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :return: The rule5 of this Datapoint.
        :rtype: bool
        """
        return self._rule5

    @rule5.setter
    def rule5(self, rule5: bool):
        """Sets the rule5 of this Datapoint.

        Denotes wether this rule was broken by the sentence  # noqa: E501

        :param rule5: The rule5 of this Datapoint.
        :type rule5: bool
        """
        if rule5 is None:
            raise ValueError("Invalid value for `rule5`, must not be `None`")  # noqa: E501

        self._rule5 = rule5

    @property
    def date_added(self) -> str:
        """Gets the date_added of this Datapoint.

        Date added to the database  # noqa: E501

        :return: The date_added of this Datapoint.
        :rtype: str
        """
        return self._date_added

    @date_added.setter
    def date_added(self, date_added: str):
        """Sets the date_added of this Datapoint.

        Date added to the database  # noqa: E501

        :param date_added: The date_added of this Datapoint.
        :type date_added: str
        """
        if date_added is None:
            raise ValueError("Invalid value for `date_added`, must not be `None`")  # noqa: E501

        self._date_added = date_added
